<template>
  <div>
    <h1 class="text-2xl font-bold text-white mb-6">{{ $t('myTickets.title') }}</h1>
    ...
    <div v-else-if="tickets.length === 0" class="text-center py-16 glass rounded-2xl border border-slate-700/50">
      ...
      <p class="text-slate-400 text-lg mb-2">{{ $t('myTickets.noTickets') }}</p>
      <p class="text-slate-500 mb-4">{{ $t('myTickets.browseLotteries') }}</p>
